...
UCountess_GameplayAbility_Regen::UCountess_GameplayAbility_Regen()
{
	...
	static ConstructorHelpers::FClassFinder<UGameplayEffect> Countess_Regen_Effect_Class(TEXT("'/Script/Project_Countess.Countess_GE_Regen'"));
	if (Countess_Regen_Effect_Class.Succeeded())
	{
		Countess_Regen_Effect = Cast<UGameplayEffect>(Countess_Regen_Effect_Class.Class->GetDefaultObject(false));
	}
...
// 			AbilitySystemComponent->AddLooseGameplayTag(FGameplayTag::RequestGameplayTag(FName("State.Buff.FullStamina")));
			ApplyGameplayEffectToOwner(Handle, ActorInfo, ActivationInfo, Countess_Regen_Effect, 1.f);
